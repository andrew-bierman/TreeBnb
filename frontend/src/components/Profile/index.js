...
import { getUserBookings, actionCreatorResetUserBookings } from '../../store/bookings';
...
const Profile = () => {
    ...
    if (userBookingsArr.length > 0) {
        userBookingsSpots = Object.values(allSpots)
            .filter(spot => {
                ...
                const bookingDatesExist = userBookingsArr
                    ...
                    .every(booking => booking.startDate !== null && booking.endDate !== null);
    ...
    const checkBookingEditAndDeletionValidity = (bookingId) => {
        ...
        const currentUTC = Date.UTC(currentDate.getUTCFullYear(), currentDate.getUTCMonth(), currentDate.getUTCDate(), currentDate.getUTCHours(), currentDate.getUTCMinutes(), currentDate.getUTCSeconds(), currentDate.getUTCMilliseconds());

        const bookingDate = new Date(booking.startDate);
        const bookingUTC = Date.UTC(bookingDate.getUTCFullYear(), bookingDate.getUTCMonth(), bookingDate.getUTCDate(), bookingDate.getUTCHours(), bookingDate.getUTCMinutes(), bookingDate.getUTCSeconds(), bookingDate.getUTCMilliseconds());
                    ...
                    <div className='is-flex is-flex-direction-column is-align-items-center is-flex-direction-center'>
                        ...
                        <div className='columns is-flex-wrap-wrap is-3 pt-5 pl-3 pr-3 w-100'>
                            <>
                                ...
                                {Object.values(userBookingsArr).length > 0 && userBookingsArr.map(booking => {
                                    const spot = userBookingsSpots.filter(spot => spot.id === booking.spotId)[0];

                                    if (!spot) return (<></>);

                                    return (
                                        <div key={booking.id} to={`/spots/${spot.id}`} className='column is-full-mobile is-half-tablet is-one-quarter-desktop'>
                                                ...
                                                <div className="is-flex-direction-column is-justify-content-flex-end mt-5 are-small">
                                                    {
                                                        ...
                                                        <>
                                                            {
                                                                (
                                                                    <div className='container has-text-centered' key={booking.id}>
                                                                        <div className=''>
                                                                            <div class="icon-text">
                                                                                ...
                                                                                <span>
                                                                                    {
                                                                                        convertISODateToUTC(booking.startDate)
                                                                                    }
                                                                                </span>
                                                                            </div>
                                                                        </div>
                                                                        {
                                                                            checkBookingEditAndDeletionValidity(booking.id) && (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <BulmaModal
                                                                                        ...
                                                                                        content={(isActive) => <EditBookingPage spotId={spot.id} bookingId={booking.id} isActive={isActive} />}
                                                                                    ...
                                                                                    <BulmaModal
                                                                                        ...
                                                                                        content={(isActive) =>
                                                                                            <DeleteBookingConfirmation
                                                                                                spotId={spot.id} bookingId={booking.id} isActive={isActive}
                                                                                            />
                                                                                        }
                                                                                        save={() => console.log('saved')}
